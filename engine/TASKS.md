...
>
> 找"现在怎么跑"请看：根 `BUILD.md` 的自检一节、`internal/dev/tzs/README.md`、
...
**W0–W3 全部走完，函数全部实现（此后补了 `reparent`，现 49 个），W3 门已跑完并通过。**语料已缩到 67 个且四条基线全干净。 结论与四个缺陷见
...
> `D:\我的项目\T100设计器\.tzs-cli\` 下）。**要跑关卡请照根 `BUILD.md` 的自检一节或
...
> **四个批脚本的基线已按 67 个文件重采（`f7283e2`）——四条现在全是 67/0/0。**
> 原先所有不干净的行都在被删掉的 `xiyuan/tst` 里：那个 `addfail`（`bsft001_wf`，
> 缺基础资料会让设计器弹模态框，只有人点掉才会过）和两个 `skip`（无可用列）。
> 所以语料现在是**全干净**的，基线比原来更强——出错就是真出错。
