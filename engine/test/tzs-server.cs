...
class TzsServerMain
{
    ...
    // third-party software that is NOT redistributed with anything we ship, so where it lives is
    // the user's answer, not ours. Not `const` because it comes from the environment.
