...
### 输出是 CSV,头部那几行注释是必须看的

默认输出**不是 JSON**:头部若干行 `# ` 注释给出**本次查询的环境信息**,其后是纯净的
CSV 主体(表头行 + 数据行)。字段里有逗号/引号/换行会被正确转义,所以可以放心按 CSV 解析;
`grep -v '^# '` 切掉注释行就是标准 CSV。
...
**账号由企业编号(TOPENT)决定,不用你操心** —— 但**"查不到数据"时先核对头部那行**:
企业编号(或环境)不对就会连到另一个 schema/另一套库、拿到 0 行,而那看起来和"数据不存在"
一模一样。实测同一个查询,企业 99 下是 368950 行、企业 907 下是 62 行 ——
认错的代价是**得出错误的业务结论**。

要机器可读的完整结构(含 `totalRows`/`truncated`/`notes`/`serverLimited` 等字段)才加
`--json`;日常查数据直接读 CSV 即可。
