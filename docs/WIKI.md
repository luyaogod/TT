...
tt dev tzs …   设计器表单包（.tzs/.tzv）：55 个具名动词 + JSON 参数（field_add 任务级 + open/save/set_spec_attr…），
               由设计器自己的引擎读写，不是拼 XML
...
| [7. `tt dev tzs` 表单包](#7-tt-dev-tzs-表单包) | 引擎与设计器程序集、55 个动词（含任务级 `field_add`）、寻址与基线规则 | 用或改表单读写时 |
...
| 能改回来吗 | 改 `prog.full.4gl` → `verify` → `apply` | **不走 export**：要用 `tzs <动词>` 让设计器自己算（§7） |
...
`tt dev tzs <动词>` 背后是 `engine/` 里一个 **C# 引擎**（`tzs-server.exe`）。它**不实现** `.tzs`
...
| `<动词> --args '<JSON 对象>'` | 读写表单，**唯一写路径**（**55 个具名动词**，见下） | 是 | 只经 `save` 写到**新**包 |
...
**函数就是动词** —— 直接敲名字，没有 `call <fn>` 这一层（由引擎的函数表生成，
命令层不存任何函数名；见 §7.5.1）。**也没有"打印整张函数表"的命令**：枚举动词靠 `--help`
（或敲错名字，会打同一份清单），参数按需靠 `<动词> --help` —— 见 §7.5.2。
按组分（`[工作流]` 排在最前 —— help/索引按 manifest 顺序打印，推荐入口应该第一眼看到；
会改模型的标 `[写]`，慢的标 `[slow]`）：
...
`wrap` 是拿选中元素包一层新容器**。`add_field` 的 `columns` 一次构造多列，`container` 给
`"Table"` 才得到"一个 Table 装 N 列" —— 一次一列会让大表单慢两个数量级（实测 84 列从 137 秒
降到 1.17 秒，而且会在 Table 里给每个控件塞一个 Label，真实表单里没有那种形态）。
...
（`Rpc.FindByKey`）。于是调用方不必把 `h1` 这种易失的随机号在每条命令之间搬运 —— 那正是
[Writing effective tools for agents](https://www.anthropic.com/engineering/writing-tools-for-agents)
点名的一条：**别把 `uuid` 这类晦涩标识符暴露给调用方，换成语义化、稳定的名字**（实测能显著
提升准确率）。这两个名字是现成的：`open` 的应答里就有 `program` 与 `key`，`list_open` 也报。

线上形状**一个字没改**：字段仍叫 `handle`、仍是字符串、`Manifest.Check` 不动 —— 这是
**允许更多写法**（SPEC §11.24(b) 的加性扩展），不是收紧契约。两种寻址只能给一个；
给不需要句柄的动词传 `--form` 会报错（那说明调用方以为它针对某张表单）。

| 情况 | 行为 |
|---|---|
| 名字命中 1 个会话 | 正常执行 |
| 名字命中多个（同名程序的不同类型同时开着） | `bad_param` + `detail.candidates`，**不猜** |
| 名字谁都不命中 | `not_found`（退 2），文案点出三种写法，`detail.candidates` 列出当前开着的（含可用的 `key`） |

底下的句柄规则**没变**（它们仍然是引擎的实现模型，只是不再是对外契约）：
...
```powershell
tt dev tzs <动词> [--form <程序名>] --args '<JSON 对象>' [--json]
tt dev tzs <动词> --args-file <UTF-8 文件>     # 长内容/中文；值是单个 - 表示读标准输入
tt dev tzs list_open --json                    # 无参数的动词可以省掉 --args
```

`--args` 的对象**就是**引擎那侧的 `args`，键名与 manifest 里声明的一模一样：

```powershell
tt dev tzs nudge --form aapp320 --args '{"paths":["a/b","c/d"],"direction":"right","offset":1}' --json
```

传输级开关（**不是**动词参数）：`--form` / `--args` / `--args-file` / `--workspace` /
`--rpc-timeout <秒>` / `--json` / `-h`。动词名可以用连字符写（`form-tree` = `form_tree`）。

**慢动词会先出声。** 超过约 400 ms（`open` 加载包、`validate` 跑校验器）先在 **stderr** 打一行
`… 正在执行 validate；还在等引擎，慢是正常的`。这是 clig.dev 那条「长操作别静默，否则看起来像
卡死」的落地；数据仍然只在 stdout，管道里不会多出东西。阈值而不是"开始/完成两条"，是为了让
毫秒级的动词保持安静（否则每次调用两个行就是噪音）。

**为什么把 flag 形式整个删掉**（曾经是 `call <fn> --handle h9 --paths a,b`）：

| | flag 形式 | JSON 形式 |
|---|---|---|
| 谁来认识参数名 | 命令行动态解析，于是"数组按逗号切、可重复、裸开关 = true、负号不是 flag"全成了调用方要学的语法 | 引擎 manifest 的键名，原样照抄 |
| 数组 / 布尔 / 数字 | 全是字符串，靠猜（`--paths "a,b"`、`--excluded 0`） | 就是 JSON 数组/布尔/数字 |
| 引号与转义 | 归 shell 管（PowerShell 还要再一层） | 归 JSON 管 |
| 参数名与保留开关撞车 | 真出过：`--timeout` 被客户端读超时吃掉，`open` 的加载看门狗**永远送不到引擎** | 不存在这个问题（参数不是开关） |
| 参数打错的报错 | 只有引擎能报（它才知道名字与类型） | 一样只有引擎能报 —— 但**本地先按 manifest 拒一遍**，见下 |

本地只做**语法**校验（未知动词 / 未知参数 / 缺必填 / int / bool / enum 静态取值 / 数组元素类型），
语义交给引擎：

| 情况 | 行为 |
|---|---|
| 未知动词 | 退 2，列出**全部**动词（按组；连字符写法算命中） |
| 未知参数 | 退 2，**一次报全部**（排序后）+ 该动词的参数名 |
| 缺必填 / `"2"` 给 int / `1.5` 给 int / `"yes"` 给 bool / 枚举越界 / 数组元素非字符串 | 退 2，文案点名参数与合法值 |
| 参数写 `null` | 视同**没给**（缺必填照报） |
| `kind` / `attr` / `add_action` 的 `type` 取值不存在 | **本地放行**，由引擎用它自己的白名单/该表单的 `s_detail<n>` 拒绝 |
| 参数写成 flag（`--handle h9`） | 退 2「未知开关」 |
| 位置参数（动词名之外的裸词） | 退 2「多了位置参数」 |
| 寻址写进 JSON（`"form":"aapp320"`） | 退 2「参数 form 不在 manifest 里」+ 明细里提醒用 `--form` |
| `--form` 与 `handle` 同时给 | 退 2「只能给一个」（不做优先级猜测） |
| `--form` 给不需要句柄的动词 | 退 2「不接受 form」 |

**本地绝不拦那三处**的理由没变：`attr` 的合法集要从**活着的模型**里取、`kind` 的七种
**不在 manifest 里**、`add_action` 的 `type` 是**每张表单自己的**词表 —— 静态判不了。
拦下来只会让一个引擎本会接受的调用**永远到不了引擎**（而且拦得静悄悄：写错了不会有任何
测试失败，只会让某些调用在真机上莫名其妙过不去）。引擎拒绝它们的退出码是
`kind=validation → 2`，与本地拦下来一模一样，所以放过不会让报错变晚或变差。

两个附带事实：**参数打错的报错也要引擎在**（命令先拉 manifest 再校验参数）；
`--rpc-timeout` **最小 120 秒**，低于此值会被抬到 120（下限是给引擎加载看门狗留的）。
引擎那个看门狗是**另一个**参数：`open --args '{"path":"…","timeout":30}'`。

#### 7.5.1 为什么是具名动词，而不是 `call <fn>`

55 个动词由引擎的函数表在运行时生成：`tt dev tzs <动词>` 里，动词名 = manifest 的 `fn`，
参数 = manifest 的 `args`。命令层**一个动词的字面量都没有**（`runTzsVerb` 只吃 manifest 条目），
所以引擎加/改函数时，`tt` 侧一行都不用改 —— 这是 §7.1「格式那部分由设计器自己在算」在
命令行上的同一条思路：**能问的就不抄**。

删掉 `call <fn>` 与 flag 形式换来三件事：

1. **没有"函数名"这一层要填。** 旧写法里 `call` 之后的名字与 `--<参数>` 都是调用方要记住的
   运行时契约；现在是动词名（敲错就报错并列出全部），参数按需问 `<动词> --help`。
2. **只有一份参数判据。** 从前有 `--k v` 与 JSON 两条入口，就得有测试钉住"两条入口产出同一串
   字节"；现在只剩一条，那类漂移在结构上不存在。
3. **保留名与参数名不可能再撞车。** 参数不是开关了，§7.5 表里那行"真出过"的事故从此没有发生条件。

代价写在明处：**"不用 shell 打一堆引号"这件事从调用方转移到了工具文档**（JSON 里要写
`\"` 或改用 `--args-file`）。对一个 AI 调用方来说这笔交易是划算的 —— 它本来就在生成 JSON。

#### 7.5.2 函数表不外发，Go 侧也不写死任何函数名

`fns`（打印动词表）与 `manifest`（原样转发 34 KB 函数表 JSON）两个命令**已删除**，并且传输层
不再借用业务函数：

| 项 | 从前 | 现在 | 依据 |
|---|---|---|---|
| 枚举动词 | `tt dev tzs fns` 打全表（名字 + 全部参数 + 类型 + 必填 + 取值集） | `tt dev tzs --help` 打**索引**（分组 + 名字）；敲错名字打同一份清单 | 参数是每个动词自己的契约，按需问 `<动词> --help`；一次摊开 55 个动词的参数只会淹没人 |
...
前两行的共同理由：**传输面与业务面别混在一起**。让就绪探测去调 `list_open`，等于把 Go 侧
钉死在引擎的一个函数名上 —— 引擎哪天改了名、改了语义（比如加了个需要句柄的校验），握手就
跟着坏，而症状是"冷启动 60 s 未就绪"，与真正的原因毫无关系。
...
`tt dev tzs <动词> --help` 按 clig.dev 的「示例优先」把一条能直接粘的命令放在参数表**前面**
（人们读示例比读文档多）。它是从 manifest 参数表生成的，**不手写** —— 手写的示例一定会漂移，
而漂移的示例比没有示例更坏。三条生成规则：

- 只列**必填**参数（例子要短、要一定能跑通；选填参数在下面的表里），例外是任务级动词的
  `file` —— 它是寻址手段，缺了示例就粘不了；
- 需要句柄的动词写 `--form aapp320`（而不是 `handle`）—— 那才是调用方想说的东西；
- 占位值按类型给（`int → 0`、`bool → true`、`enum → 它的第一个合法值`、`path → "<name-path>"`，
  `file` 例外地给 `"<包路径>"`：两者都是 `t=path`，但说错了会让人填一个 name-path 进去），
  拼 JSON 用的是**与线上帧同一个转义器**（`tzs.JSONString`）—— 演示的写法与真正发出去的写法
  不一致，本身就是一类 bug。

E2E 里对真 manifest 的**每一条示例**逐个断言「`--args` 那段是合法 JSON、需要句柄的必须带
`--form`」：生成的东西才敢这么验。

#### 7.5.4 任务级动词：`field_add`（以及为什么它值得放进引擎）

§7.5.1 说过每个动词都对应引擎的一个函数。`field_add` 不同：它对应**一串**动作。这不是把
CLI 写胖了，而是把**调用方本来就要手工做的那段**搬进引擎 —— 有实测数字支撑。

**任务**：给真实表单 `apmt500_wf(c).tzs`（290 字段）按 `pmdl_t` 加 3 列并另存新包。

| | 调用次数 | 输出 | 耗时 |
|---|---|---|---|
| 手工链（`list_columns` → `open` → `list_spec_nodes` → `form_tree` → `validate` → `add_field` → `validate` → `save` → `close`） | **9** | **62,873 B** | 15.7 s |
| `field_add` | **1** | **1,379 B** | 13.7 s |

省下来的**不是时间，是调用次数与 context**（46×）。时间省不掉是因为校验本身必须跑：手工链里
两次 `validate` 占 13.3 s / 15.7 s（85%）；`field_add` 第一次调用同样要跑两次（一次**在改动前**
建基线，否则"增量"按构造恒为空，§7.4），但它把这段关进了一次请求，第二次对同一张表单就用上
缓存了（`baselineCached:true`）。

它顺手关掉的三个坑：

| 坑 | 手工链上真实发生过 | `field_add` 的做法 |
|---|---|---|
| `E_KEY_IN_USE` | 上一条命令把表单开着没关，下一条 `open` 就退 4（写基线时踩到过） | `file` 已在开着就**复用**（`Rpc.FindByPath`），返回体里 `opened:false` |
| 容器 path 要先探 | `form_tree` 下钻看结构 | 规则化自动挑：`worksheet` → 名字含 `layout` → 根下唯一容器 → **挑不出来就报错要 `into`**（不猜） |
| 校验基线 | 得记得先跑一次 `validate`，否则增量是假的 | 引擎自己在改动前建（已有缓存就不重复建） |
...
**`--json` 下 stdout 的规则：一次动词调用恰好一帧**，形状永远是 `{id,ok,result,error,ms}`。
引擎发的与客户端合成的**是同一个结构体**（传输失败、manifest 拉不到、参数定型失败、
守护进程起不来都会合成一帧 —— `tzs.SyntheticFailure`），所以消费方不必分辨是谁发的；
...
| 例外 | 为什么 |
|---|---|
| 用法屏：没有动词、动词名打错（打的是动词索引）、`<动词> --help` | 那是**说明书**，形态由说明书决定，不是调用结论 |
| 参数形状错：`--args` 与 `--args-file` 同时给、多了位置参数、旧 `call` 写法 | 在拿到 manifest 之前就看出来了，谈不上"这次调用" |
| 环境没就绪：引擎 exe 找不到、工作区没配、`--args-file` 读不到 | 连"要跑哪个动词"都还没算出来 |

由这三条例外得到一条**只往一个方向成立**的判据，而它是这个规则真正有用的那一半：

> **stdout 为空 ⟹ 这次调用没有发出任何请求**（可以放心改参数重来）。
> 反过来不成立 —— manifest 拉不到也会给一帧，所以"有帧"不代表"发出去过"。
> 唯一需要当心的是**帧里 `code=E_SERVER_DIED`**：请求可能已经上线、只是应答没回来，
> 那一条**绝不重试**。

（`tt dev` 组自己就是完整的 CLI，所以根命令不再替它抄一份失败信封 —— 否则 stdout 上会出现
两个对象。见 `internal/cli/root.go` 的 `alreadyReported`。）

**红线**：`export` 的产物是**只读参考**，不要手工改完再塞回包，也不要拿它当 `tzc` 工作区去
`apply`；`save` 的 `out` **永远指向新包**，绝不指向源包（**引擎现在会拒**，见 §11.9 第 1 条）；
**请求一旦上线绝不重试**（协议无幂等键，重试是在赌"上一次写进去了没有"）。
...
### 9.3 语料 pin 的漂移，与一次"证明闸门有用"的事故（2026-09-24）

**先说结论：`TestCorpusPin` 现在是红的，而那不是引擎改动造成的。**

`engine/corpus.manifest` 钉着 67 个包，语料根下实际有 **70** 个 `.tzs`。多出来的三个是
`109/35/bsft001_wf(s).tzs`、`apmt500_wf_attrdemo_test.tzs`、`apmt500_wf_gridadded_test.tzs`
（后两个看名字是早期引擎工作的试验产物，从未入册）。pin 的判据是"**发现出来的语料与 pin 逐条
一致**"，所以这条用例在这台机器上一直红着 —— 它归 `TTZS_DEEP`，默认不跑，于是没人看见。
两条路都得显式做：把那三个文件挪出语料，或重新生成 pin（`engine/make-manifest.sh [root]`）。
**本轮没做**：它们是语料还是垃圾，是仓库主人的判断，不该由工具替它裁决。

**再说事故。** 给 `out` 闸门做"撤掉它会不会被测试抓住"的验证时，撤掉闸门那一次重建之后跑
`TestOutGateRefusesToDestroyTheSource`，它当场把 `109/35/bsft001_wf(s).tzs` **覆盖**了
（sha256 `3258ac73…` → `f6577d9d…`，34488 字节）—— 正是这条闸门要防的那件事，只不过这次是
测试自己干的。善后与判断三条：

- **它不在 pin 里**，而且 pin 里那 67 个包**一个字节没动** —— 证据是 `TestCorpusPin` 的失败
  输出里只有"多出 3 个 / 数量 70≠67"两条，**没有任何一条 `语料在回归底下变了`**（那条判据对
  每个 pin 过的文件都比 sha）。
- **它仍然是一个有效、自洽的包**：`RoundTrip` 复核 `env=s tpl=F spec节点 853→853 布局元素 428
  stale=1`（`stale=1` 是 pristine 就有的那一类，语料里另外三个包同样如此）。
- **原始字节找不回来了**（`109/35` 不在任何一份副本里，语料也不是 git 仓库），而 `save` 对**未
...
- **打包脚本只采集、不构建**：`build_portable.bat` 按名字采引擎那三个文件与
  `engine\designer\`（`engine/out/` 里还有十几个探测程序，xcopy 整个目录会把它们一起打进包里）。
  **引擎自带的 C# 客户端 `tzs-cli.exe` 不进包** —— 对外只有 `tt` 一个入口，两个客户端就是两套
  命令面与两套退出码（§7.6）；它仍照编，引擎侧验收 harness 用它（见 `engine/BUILD.md`）。
...
第 15 条（`describe_kind` 报类型与取值集）是第二轮评测逼出来的；第 16 条（`added[]` 带绑定信息）
与第 17 条（`list_packages`）是第三轮评测逼出来的。全部同日发。第 18 至 20 条
（dry-run / reload / op log）是 2026-09-26 那一批，**其中第 18 条的落法在路上被实测推翻了一次**
（原文写的是"批量 undo"，见该条）。这一批至此全部完成。**
...
1. ✅ **`save` / `field_add` 的 `out` 闸门（已做，2026-09-24，`Fns/Session.cs` 的 `CheckOutPath`）。**
   ...
   SKILL 的红线里、代码零防线**。现在在写之前拒两件事：
   ① `out` == **任何已打开**包的路径 —— 不只是正在存的那个，`save --form A --out <B 的路径>`
   会毁掉 B；② `out` 不在工作区之下（镜像设计器**加载**时那条字符串前缀规则，含"两边各补一个
   尾 `\`"，所以 `…\ttwsX` 不会被当成 `…\ttws` 的子路径）。两者都回 `E_BAD_PARAM` +
   `detail.reason`（`out-is-open-package` / `out-outside-workspace`），退 2、可自纠。
   ⚠️ 工作区那一侧读的是 `Designer.Workspace`（引导后的权威值）：第一版只读 `Rpc.WorkspaceHint`，
   而那个字段在守护进程分支里**从不赋值**（只有 `--pipe-name` 那条路设它），于是那半条闸门
   被自己静默跳过了 —— 实测发现，不是推理出来。
   验法：`TestOutGateRefusesToDestroyTheSource`（语料层；判据三问 —— 拒源包且**源包 sha 不变**、
   拒区外、**放行**区内新路径。第三问是防止"见谁都拒"的实现让前两问变绿）。
...
10. ✅ **`list_local_strings` 的过滤器（已做，2026-09-25）—— 而它牵出的是一整类，见第 13 条。**
    ...
    （`Read.cs:859`）—— 那个名字从未声明过，于是"按关键字过滤多语言串"**从线上够不到**。
    这个问题"它到底该不该有过滤器"已经有答案：**该有**，因为 SKILL §4.7 立的规矩正是
    "清单类动词先过滤"，而它在一张 670 元素的表单上正是大块头（实测 `aapt300` 全部 **337 条 /
    13,807 字节**；`query:"lbl_apca"` → 84 条 / 3,312 字节；再叠 `path` 限定子树 → **6 条 /
    398 字节**）。改法与它的两个兄弟**逐字相同**（`list_tables` / `list_columns`）：
    `query` 放线上、`filter` 留作进程内库调用方的别名，两个名字都要读。
...
13. ✅ **把第 10 条当成"一类"来量，又牵出四类（已修，2026-09-25）—— 这一次是量出来的，不是看出来的。**
    第 10 条的形态是"函数体读了一个 manifest 没声明的名字"。这类事**手工能不能找干净是可疑的**
    （第 6 条就是手工表的，漏了一个；第 11 条又是它漏的那个）。所以写了个抛荒脚本：解
    `Manifest.cs` 里每个 `F(...)`/`N(...)` 块的声明名，解 `Fns/*.cs` 里每个 `Register` 到的
    函数体读到的名字（认 `Arg`/`Need`/`NeedKey`/`IntArg`/`BoolArg`/`ListArg`/`Obj`/`Str`/`Bool`/`Int`
    这些取值助手，连 `Read.` 前缀在内），两个方向对一遍。那一批动词的体全解析出来了。结果四类：
    - **`get_component` 读 `query` 而声明里没有，且 `path` 被声明成必填。** 函数体明写
      "需要 path 或 query 之一"、`query` 走的是 `find_component` 那同一个解析器 —— 可调用方连
      "不给 path"都做不到，本地就先拒了。**同一个病在第 7 条那批里刚修过一次**
      （`set_spec_description` 的 `kind`），这是第二次。现在 `path` 可选、`query` 声明上。
      实测 `get_component --args '{"query":"b_apcadocno"}'` 一次拿到那个 Edit 的
      table/column/fieldType（从前要两次调用）。
    - **`list_spec_nodes` 读 `name` 而声明里没有。** "先过滤"这条规矩在这个动词上不可执行，而它是
      **答案最大的那个**：`aapt300` 不过滤 **701 个节点 / 29,918 字节**，`query:"app"` →
      **11 个 / 555 字节**（54 倍），`kind:"field"` → 269，两者叠 → 6。改法与第 10 条同形。
    - **六个走 `Struct.PathsArg` 的结构动词里，五个只声明了必填的 `paths`。** 那个助手同时收
      `path`（单个）、`path`（数组）、`paths`，实在都没有才说"需要 path 或 paths" —— 于是它自己的
      错误文案点名了一种 CLI **必拒**的写法。第六个（`delete`）是另一个方向的歪：声明了必填的
      `path`，于是只想批量删的人也得塞一个 `path` 才发得出去。现在六个一律 `AnyPath()` +
      `AnyPaths()`（都可选、都带 role），"至少给一个"由函数体判 —— 与第 8 条 `add_field` 的
      `column`/`columns` 同一个理由：`Required` 是按参数打的，表达不了二选一。
      实测 `move --args '{"path":…,"to":"last"}'` 过得去，`--args '{"to":"last"}'` 退 2 并说
      "需要 path 或 paths"。
    - **`validate` 与 `verify` 的 `path` 只会回 `E_NOT_IMPLEMENTED`。** 声明写着"只校验这条子树"，
      函数体（`Validate.RejectPath`）只能把它拒掉：设计器的校验器收的是整份 `FormElement` /
      `TSDElement`，没有子树入口。而 `E_NOT_IMPLEMENTED` 的 kind 是 `internal`（"上报、别重试"）
      —— 一个 `--help` 主动提供的参数，用了却拿到"请报 bug"；`tt` 侧"先收窄：path 能减少返回"
      那句提示还会把人推过去。这与 `force`（第 4 条）是同一种东西，处置也相同：**从 manifest 删掉
      声明**，函数体里的拒绝原样留着当兜底（直接调库的人仍拿到诚实的理由）。实测
      `validate --args '{"path":…}'` 现在是本地「参数 path 不在 manifest 里（函数 validate）
      （该函数的参数：handle）」，退 2。

    收尾状态：**两个方向都干净** —— 没有动词读未声明的名字，也没有声明了不读的参数。六个"读了
    未声明"的名字在白名单里，每一个都有出处与理由（`filter` / `name` / `attr` / `column` 是进程内
    库调用方的别名；`outPath` 是 SPEC §11.24(b) 散文里的写法，线上名是 `out` 且先读它；`force` 是
    第 4 条留的兜底拒绝）。**脚本是抛荒工具，没入库**：它按正则解 C#，误报过好几次
    （`OptEnum`、`PathsArg(a, "delete")` 的第二个参数是**动词名**不是参数名、两个文件里的
    `Run` 重名、`field_add` 用另一个建表助手 `W`），所以它的结论只用来**指路**，每条都由人读源码
    复核过。要让它入库，得先把它写到那批动词都骗不过它 —— 这条提醒留给下一轮：
    **这类普查给的是可信的线索和不可信的全集，别把它的绿当成证明。**

14. ✅ **`save` / `field_add --out` 返回写入字节的摘要（已做，2026-09-25）—— 由第二轮评测的 F10 逼出来的。**
    那一轮把最贵的一条摩擦量了出来：文档把"回读"定为证明"改对了"的**唯一**手段，而回读要
    **三次引擎调用**（`save` → `close` → `open 新包` → 读）—— 因为 `get_component` 与 `verify`
    读的都是 save 写盘所依据的那个**内存模型**，拿它们回读是**自证循环**。四个执行者里三个
    各自想出了"close 再 open"这一步，没有一个是从文档里读到的。
    改法只动返回体：`save`（`Fns/Session.cs` 的 `SaveFn`，`field_add` 的 `saved` 就是它的返回）
    现在多两个字段 —— `sha256`（刚写进 `out` 的那份字节的摘要）与 `key`（新包会占用的 ProgramKey，
    与源包相同），外加一句 `note` 把这两件事说给人听。于是：
    "**落了盘没有**" = `sha256sum <out>` 与摘要比一次（**一条 shell 命令，不碰引擎**）；
    "**盘上内容是什么**" 才需要 close → open → 读。
    它为什么成立：save 渲染的就是当前模型（§11.24(b) 的不动点，语料回归在每个包上验它），
    而摘要就是摘要 —— 两个已知事实一叠就闭合了。SPEC §11.24(b) 里写下了这个链条。
    防线：语料回归的 `saveTo` **就地**断言"返回的 sha256 == 盘上那份文件的 sha256"，
    每个包跑一次（70 个包）；实测摘要与 `sha256sum` 逐字相同。
...
16. ✅ **`field_add` 的 `added[]` 里层节点带上绑定信息（已做，2026-09-25）—— 第三轮评测 F20。**
    那一层从前只有 `{path,name,tag}`，于是"哪个节点绑到哪一列"在返回体里**看不出来** ——
    执行者只能靠节点名（`pmdl_t.pmdlent`）猜，或者干脆回读一次。而引擎**本来就知道**：
    元素的 `sqlTabName` / `colName` 就在模型里，`find_component` 报的正是它们。
    改法就是用**同一个取用点**（`Json.cs` 读它们也在这两行）：字段节点多两个键，配套 Label
    两个都没有 —— **缺席本身就是信息**（它是同伴，不是字段）。
    实测：两个字段节点各带 `table:"pmdl_t"` 与自己的 `column`，两个 `lbl_*` 什么都不带。

17. ✅ **`list_packages`：工作区里有哪些表单包（已做，2026-09-25）—— 第三轮评测 F23。**
    在这之前"哪个文件是 aapt300"只有一个答案：**自己 `ls` 工作区** —— 真实工作区里那是
    **29.9 KB** 噪音（`.tzc`/`.bak` 混着），三个执行者独立撞上、各自手工过滤。现在它是一条
    与 `list_tables` 同族的读动词（**不需要句柄**，答案来自文件系统）：`query` 是文件名子串、
    `limit` 默认 200，回 `{workspace,count,returned,truncated,packages[]}`，每条带
    `name`/`path`/`program`/`sizeBytes`/`modified`。
    **两条刻意的边界**：① 它**不打开包** —— `program` 是从**文件名**推的（`<程序名>(c).tzs`
    那条约定），是挑文件的提示，`open` 才是权威（实测两者一致，但设计上不依赖它）；
    ② 它只列 `.tzs`/`.tzv`（表单侧），`.tzc/.tzf/.tzx` 是另一条管线（`tt-dev-tzc`）。
    扫不动工作区时回的是 `E_BAD_PARAM`（"环境不对"，退 2），不是 `E_INTERNAL`（"上报 bug"）。
    实测：`ttws` 那个工作区 **67 个包 / 11.8 KB**（`ls -la` 是 29.9 KB），`query=aapt` → 5 条，
    `limit=3` → `truncated:true`。

18. ✅ **写动词的 `dry_run`：跑一遍、留下答案、把模型放回去（已做，2026-09-26）—— 而它的落法
    在路上被实测推翻了一次。**
    记在这里的原文是「应用 → 读 diff → 批量 undo，撤销栈是现成的」。**逐个动词量过之后，
    那句话不成立**：撤销栈管的是设计器的 `UndoRedoCommand`，而写动词的相当一部分**写在命令之外**
    —— `UICreator` 是工厂不是命令，它建进模型的规格节点从来没进过栈。判据是
    `save → dry-run → save` 比 sha256，拿 15 个写动词在 aapp320 上量过，结果分成两半：

    | 逐字节回得去 | 只动布局的四个：`set_layout_attr` / `insert_at` / `nudge` / `fit_size` |
    | 留残留 | `set_spec_attr`（多一个 `status="u"`）、`rename_component`、`add_widget`、<br>`set_local_string`、`set_excluded`、`convert_widget`（回滚 `complete:false`）、<br>`add_field` / `field_add`（多出整套规格节点 + 一条 `TBinding`，`.bdx` 跟着长） |

    也就是说：**干跑会改了你的表单而不说**。于是回滚换成一条构造上就精确的路 ——
    ① 把**当前模型**渲染成一份临时包（`_tt_dry_<pid>_<n>.tzs`，写在源包**旁边**，因为
    TzpManager 只在工作区内加载）；② 跑动词；③ 从句柄上摘掉会话、**从那份临时包重读回来**，
    句柄串不变（复用第 19 条的 `ReKey`）。回滚因此就是"reload 到你渲染出来的那一刻"。
    改完之后**全部 21 个能驱动的动词逐字节回得去**，语料回归 `TestCorpusDryRunRollsBack` 在
    **65 个包**上全绿（判据里还有一条非空转的：同样的参数真写一次，产出必须变）。

    **代价写在返回里，不藏着**：会话被重建（`loadMs` 重计、`state` 回到 `Loaded`）。
    返回体是 `{dryRun:true, verb, handle, preview:{…本次本该返回的东西，逐字…}, reverted:{mode,
    complete, renderMs, loadMs, sha256}}` —— `preview` 是**键**不是把动词的字段抬到顶层：
    抬上去就会出现 `applied:true` 与"什么都没落地"同框，那正是评测一直抓的那类缺陷。
    实测开销：render 2–11 ms + load 70–350 ms（aapt300 最大），即与一次 `open` 同量级。
    `save --dry-run` 是另一条路（它不改模型，只是**不落盘**），所以它的 `reverted.mode` 是 `none`。

    **落地时才发现的四个坑**（都写进了判据，免得下一轮重犯）：① 干跑的临时包第一次没删掉时
    句柄的 `Path` 还指着它 → 下一次调用 FileNotFound（现在 `fresh.Path` 拨回源包）；
    ② 回滚必须把**句柄那个 writer** 也换成快照的（`Fns.Session.InstallLayout`）—— 否则模型回来了
    而 `.4fd` 文本没回来，下一次 save 会带上干跑那次布局改动；③ 取快照要写文件，所以它会因
    环境失败（只读目录、盘满），第一版让这个异常逃出调度器**把守护进程打死了** —— 现在它和
    动词体走同一个 catch；④ `field_add` 在**体内**解析自己的会话（`file` 分支），调度器那一刻
    没有句柄可标 —— 第一版于是**什么都没回滚**：它把列真加上了，却回 `undoCommands: 0`。
    现在是"谁先拿到会话谁 arm"（`DryRun.Arm`），而这一条正是上面那条语料判据抓出来的。

    **第五件不是"坑"而是"没查清"，如实记下**：全语料跑里出现过一次，65 个包约 14 个的干跑在
    回滚时**打不开刚刚写下的临时包**（同一进程、两条语句之前才写、`File.WriteAllBytes` 已经返回），
    而单独跑那条判据 65/65 干净、再跑也不复现，没找到是谁删的。查不出原因不等于能把承诺压在
    它身上：留底的字节本来就在手上，所以快照丢了就**重写一份再打开**，返回里多一个
    `reverted.recovered: true` —— 这台机器的事实不该藏。写完之后立刻验一次 `File.Exists`，
    它只在失败分支上报，把"我们从没写成"与"写成了、被别的东西吃了"分开。
    `_tt_dry_` 已加入 `engine/make-manifest.sh` 与 Go 侧 `scratchPrefixes` 的排除集：进程崩在
    回滚中间会留下这个包在工作区里，而它绝不该进语料基线。

19. ✅ **`reload`：丢弃内存改动、从盘重读同一个包（已做，2026-09-26）。**
    替代 `close` + `open`，而老路有一笔看不见的账：close 会 `Validate.Forget`，于是"这一轮改下来
    新增了什么"要重问一遍（大表单上一次 validate 十秒级）。reload 的语义是**同一个文件**的重读，
    所以基线在文件没变时依然成立 —— 而"文件变没变"是**量出来的**：`open` 时记下源包字节的
    sha256，`reload` 时再算一次。返回里 `baseline` 是三态而不是布尔：`kept`（字节相同，基线留着）
    / `dropped`（文件被换过，基线测的是另一份字节，必须丢）/ `none`（这个句柄还没 validate 过）。
    顺带把一件从前**完全看不见**的事变成一句话：**盘上这个包在你打开之后被谁改过**
    （`fileChanged:true`），此时调用方手里的一切对照（基线增量、diff）都在拿两个不同的文档比。
    句柄串不变（新铸的号作废不用），`discarded` 报出丢掉的撤销步数与布局是否有改动。
    文件读不出来时**就地拒绝、句柄原样**；close 之后 open 失败（包被换成了坏字节）才真的丢句柄，
    那时如实说句柄已关、并给出重新 open 的路。
    语料回归 `TestCorpusReloadKeepsTheBaselineAndDropsTheWork` 在 65 个包上把三个分支都验过
    （换文件那一支在**自己拷出来的副本**上做，绝不动语料原件）。

20. ✅ **幂等 op log：`op` + `list_ops`（已做，2026-09-26）。**
    从前只有一句禁令「请求一旦上线绝不重试」—— 那条治的是**重复**，没治**不知道**：超时之后
    既不能重发、也问不出"到底写进去没有"。现在写动词（36 个）与 `save` 都收一个 `op` 名字，
    引擎在**跑动词体之前**就把一条 `pending` 记进日志，跑完改成 `ok` / `error`（带 `code` 与
    一句摘要：`sha256`/`out`/`noop`/`newErrorCount`…；`field_add` 的 `saved_*` 也提出来）。
    于是超时之后有**三态**的机械答案：**没有记录** = 请求从未到达（重发安全）；**`pending`** =
    看见了、还在做（别盲目重发）；**`ok`/`error`** = 已经结束（重发安全，且知道结果）。
    `list_ops` 与 `list_open` 同组同级（`op` 精确匹配 + `limit`，新的在前），返回里带
    `daemonStartedAt` —— **日志只记本进程**（不落盘：它的答案只在守护进程活着时有意义，而落盘
    会把运维状态塞进客户工作区或临时目录），所以"换个守护进程问同一个 op"必须能自己看出来。
    上限 500 条，挤掉的旧条目计数并报（`dropped`），`seq` 会跳号。

    **一个落地时才发现的同名坑**：`list_ops` 自己的过滤器就叫 `op`，而调度器第一版是**按
    "参数里有没有 op"** 来认写动词的 —— 于是那次查询把自己当成一次写操作记进了日志（实测：
    查 `op=never-happened` 返回 `count:1`，就是它自己）。修法不是在名字上打补丁，而是在 `SpecFn`
    上立一个 `Traced` 标志：**声明与行为同源**（`Manifest.Traced` 挂参数、`Rpc` 读这个标志）。
    判据一分为二：manifest 那一半（带 `dry_run` 的必须是写动词或 `save`）在
    `dryrun_test.go`，行为那一半（读动词带着 `op` 不长日志）在 `oplog_test.go`。

不需要重建引擎的（Go 侧，可单独发）——**2026-09-25 一口气发了五条，全部已发**：

- ✅ **`tt dev` 收下 `--format` / `--json` / `--csv`（已发，2026-09-25）。** 根命令把这三个写作
  全局开关，可 `tt dev tzc selftest --format csv` 与 `tt --format csv dev tzc selftest`
  **两种位置**都只得到一句 tdev 的「未知子命令 "--format"」（本命令组是 `DisableFlagParsing`，
  开关会原样落进 tdev 自己的解析器）。现在翻译成这条线认识的东西：`json` → `--json`、
  `table/text` → 丢掉（缺省就是人读文本）、`csv` → **明确拒绝**（这条线没有 CSV，而"要 CSV
  拿到 JSON"正是最该避免的静默走偏）；`--env/--conn` 同理拒绝并点名它属于哪条线。
  ⚠️ 翻译出来的开关要**挂在参数末尾**：`tt --json dev …` 里那个 `--json` 本来在最前面，
  就地保留的话 tdev 会把 `args[0]` 当成子命令名（实测）。
- ✅ **root help 的退出码总表（已发，2026-09-25）。** 连同它一起修的是这张表暴露出来的两个
  真缺陷：**未知子命令退 0**（`tt debug nope` / `tt dict nope` 都打一份帮助就退 0，而 dict
  自己的契约写着「1 = 用法错」——"我打错了命令"读成成功是调用方最难自己发现的失败形态），
  以及 `tt dict`（裸）在没配库的机器上打的是"本地数据库文件未找到"而不是命令列表
  （组的 `PersistentPreRunE` 抢在 `RunE` 前面开数据源；现在落在组命令自己身上时不碰数据源）。
  表里写清"同一个数字在各家含义不同"：`3` 在 tzc 是校验失败、在 dict 是缺表、在 tzs 不用。
- ✅ **dict 截断结果的翻页入口（已发，2026-09-25）：`tt dict spill list|show`。** 截断时完整结果
  **已经落盘**（`<配置目录>/spill/query-<时间戳>.json`）且信封里给了 `localPath`，但从前没有
  入口 —— 要看全量只能重查一次（远程库可能很贵）或自己 cat 那个文件。现在 `spill show
  <文件|latest> --offset N --limit M` 按页读盘上那一份，**不重查、不连数据源**；`--limit` 沿用
  查询那一套（0 = 不限），信封里多一个 `offset`，`notes` 直接给出下一页的命令。截断那句 note
  也改成指向它（重查从首选降为备选）。⚠️ 翻页输出的**列序按字母序** —— 落盘的是数据，原始列序
  不在文件里（这条写进了 SKILL）。
- ✅ **`reap` 不再要求工作区（已发，2026-09-25）。** 它遍历的是状态文件里**每一条记录自己的**
  workspace，当前配置里有没有工作区与它要收哪些进程无关 —— 而"没配工作区就不许开工"那条保护
  是给**要 spawn** 的命令立的。从前它走严格版，于是"引擎重编过、旧守护进程收不掉"这件事发生在
  **恰恰最需要它的场合**（换机器、换工作区、刚把配置改坏）。实测：无工作区的配置下
  `reap` 照跑、`stop` 仍拒（后者按当前工作区寻址，拒得对）。
- ✅ **抓函数表没有期限（已发，2026-09-25）** —— 见下面那条独立记录（它带出两个 bug）。

**还欠着的两条都要动引擎**，所以留在这一档里等下一次重建 —— **两条都已在 2026-09-26 做掉**
（见本文档的第 18、19、20 条）：

- ✅ **`.tzs` 写动词的 dry-run**（落法原文：应用 → 读 diff → 批量 undo，撤销栈是现成的）
  —— **那句话是错的，见第 18 条**；与它一起的 **`reload`** 见第 19 条。
- ✅ **幂等 op log**（写动词带一个 `op` 名、引擎记进日志、加一个 `list_ops`）→ 第 20 条。

**一条设计过、没做的**（记下来免得下一轮重新想一遍）：
`stop`/`reap` 够不到**没有任何记录**的守护进程（C# 探针程序起的那些）。能做，但要按**进程枚举**
找（列出所有 `tzs-server.exe`、比它们命令行里的管道名与当前 MVID），而且要一条安全规则 ——
一个"MVID 与当前不同"的守护进程可能正被**另一个 tt 安装**用着，杀错了就是别人的现场。
没有那条规则之前不开这个口子。

**第三轮评测又添两条引擎侧欠账** —— **两条都已在 2026-09-25 做掉**（见本文档的第 16、17 条）：

- ✅ **`field_add` 的 `added[]` 里层节点补上绑定信息** → 第 16 条。
- ✅ **加一个"列出工作区里有哪些包"的动词**（`list_packages`）→ 第 17 条。

- ✅ **抓函数表没有期限（已发，2026-09-25）—— 一次真事故。** `tzsCtx()` 是
  `context.Background()`，而 `FetchManifest` 会 spawn `tzs-server.exe --manifest`
  —— 引擎 exe 卡住时（半死、被杀软挂着），**每一条** `tt dev tzs <动词>` 都会永远挂着，
  包括 `<动词> --help`。那次表现为 `--help` 的测试挂满 10 分钟被 go test 判超时。
  现在两处拉取各有期限（`fetchManifest`）：派发前校验参数那条 60 s（拿不到就退 5），
  `--help` 末尾那段动词索引 10 s（它是装饰，拿不到就少一段）。
  **同一次事故还翻出一个测试装置的 bug**：`captureStdout` 是"先跑完再读"，而进程内管道的缓冲
  只有约 4 KB —— `--help` 在引擎可达时输出 4,574 字节，恰好越界，于是写端阻塞、读端在等它返回。
  改成边写边读，并加了一条 64 KB 的防线。两个 bug 叠在一起才显形：**没有那次环境变量恰好把
  引擎指通，谁都遇不到**。教训记在这里：测试里的"输出量小"是有边界的断言，边界在哪要写出来。
...
- `.tzs` 写动词的 **dry-run**（落法：应用 → 读 diff → 批量 undo，撤销栈是现成的）与
  **`reload`**（丢弃内存改动、从盘重读，把"只能 close + 重新 open 且丢掉 validate 基线"
  变成一条命令）。
  ✅ **两条 2026-09-26 已做（第 18、19 条）—— 但上面那句落法是错的**：撤销栈不是模型的全部状态，
  实测只有四个纯布局动词回得去。dry-run 的回滚因此改成了"渲染成临时包 → 跑一遍 → 从它重读回来"。
- **幂等 op log**：现在只有"请求一旦上线绝不重试"这条禁令（协议无幂等键）；轻档做法是写动词
  带一个 `op` 名、引擎记进会话的操作日志，再加一个与 `list_open` 同级的 `list_ops`，
  让超时后"到底写进去没有"有机械答案。
  ✅ **2026-09-26 已做（第 20 条）**：日志在守护进程内存里（不落盘），返回里带 `daemonStartedAt`
  让"换个守护进程问同一个 op"看得出来。
- **`stop` 与 `reap` 都够不到"没有记录"的守护进程**（实测 2026-09-24）。语料回归里由**探针
  程序**起的 `tzs-server`（引擎的 C# 客户端起的，不是 `tt` 起的）**不写** `tt` 的状态文件，
  于是 `stop`（按当前工作区寻址）看不见它、`reap`（只遍历状态文件里的条目）也看不见它 ——
  两个都在跑的孤儿最后只能手工 `taskkill`。
  ⚠️ **别把"收不动"一律归到这一条**（2026-09-25 更正，同日实测）：语料回归之后剩的那一个
  守护进程是**记录在案的**（状态文件里 pid/pipe/since 齐全），`reap --yes` 不收它**是对的**
  —— 它的管道名（含 MVID）与当前 exe 一致，它不是孤儿；它该由 `tt dev tzs stop` 收，
  实测 `stop` 一收即空。所以顺序是：先看状态文件里有没有这一条，有就用 `stop`，
  没有才轮到这条"没人记录它"的路。附带一条：`reap` 在**没配工作区**时直接拒跑
  （"引擎的缺省工作区是一个真实客户目录，所以这里拒绝启动而不是回落"），可它内部遍历的是
  **记录里的** workspace（`e.Workspace`），与当前配置无关 —— 那一步的检查要么去掉，
  要么只对"要新建东西"的命令生效。
- **评测回归网**：一组任务（改一个属性并确认、按表加字段、加页签、定位一个只读区为何不能改…）
  ...
  **四轮都跑完了**（2026-09-24/25），装置与提示词在 `tools/eval/`，全部结果与摩擦记录在
  `docs/eval-baseline.md`。调用数逐轮：**58 → 62 → 52 → 48**。
  两轮的经验合起来是一条：**文档的缺陷每修一处就少一次调用，文档的建议每加一条就多几次调用
  —— 而后者买到的是验证更深**（第四轮 48 次里有 4 次是新写进文档的勤快：源包前后各一次
  `sha256sum`、两次 `list_columns`）。另一个反复出现的产出是**"不是缺信息，是信息打架"**：
  第三轮 E1 因为 §7 自相矛盾多打一次注定失败的调用；第四轮又抓出 §2 关于 `baselineCached`
  的说法**是反的**（代码里 `field_add` 是先建基线再加字段）。
  第三、四轮的摩擦（F17–F42）都已当场修掉。
  第二轮沿这条线挖出的两件事**都已做掉**：
  ...
  **第五轮的候选**：第 18–20 条（`dry_run` / `reload` / `op`+`list_ops`）刚进 SKILL §6，
  它们值不值那几次调用正是该量的东西 —— 尤其是 `dry_run`：它每次要花掉一次会话重建
  （≈一次 `open`），而语料判据只证明它**准**，证明不了它**值**。
