...
13. ✅ **把第 10 条当成"一类"来量，又牵出四类（已修，2026-09-25）—— 这一次是量出来的，不是看出来的。**
    ...
    这些取值助手，连 `Read.` 前缀在内），两个方向对一遍。52 个动词的体全解析出来了。结果四类：
    ...
    复核过。要让它入库，得先把它写到 52 个动词都骗不过它 —— 这条提醒留给下一轮：
...
**第三轮评测又添两条引擎侧欠账**（都要动引擎，记在这里免得丢）：

- **`field_add` 的 `added[]` 里层节点不带 `table`/`column`**：那一层只有 `{path,name,tag}`，
  所以"哪个节点绑到哪一列"在返回体里看不出来（第三轮 E3 实测）。文档只能教"回读去看"
  （`find_component` 会给出 `table`/`column`/`fieldType`），真解决要在引擎侧给内层节点补上。
  这是"引擎知道却没说出来"的又一例 —— 与第 14、15 条同族。
- **没有"列出工作区里有哪些包"的动词**：第三轮三个执行者都被 `ls <工作区>` 打了（实测 29.9 KB，
  几十个包混着 `.tzc`/`.bak`）。文档只能教"按程序名拼 `<程序名>(c).tzs`"，而一个列举动词
  （读工作区的包目录/登记）会让这件事变成一条命令。它在引擎侧不难，但要定"列什么、
  是谁登记了这些包"。
