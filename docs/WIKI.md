...
各说各话"当**一类**量了一遍，又修掉四处同类。这一批至此全部完成。**
...
- **评测回归网**：一组任务（改一个属性并确认、按表加字段、加页签、定位一个只读区为何不能改…）
  ...
  **两轮都跑完了**（2026-09-24 基线 / 2026-09-25 引擎批次之后），全部记录在
  `docs/eval-baseline.md`。第二轮沿这条线又挖出两件**需要动引擎**的事：
  - **"证明改动落了盘"要三个调用**（`save` → `close` → `open 新包` → 读），而它是文档指定的
    **唯一**证明手段（`get_component` / `verify` 读的都是内存模型，拿它们回读是自证循环）。
    轻档修法是 `save` 的返回里带上 `bytesOut` 的 sha256（现在返回体里没有任何可比对的指纹，
    而 `bytesOut` 比 `bytesIn` 小一截，看着像丢数据）；再进一步是一个 `verify --on-disk`
    之类的动词。顺带：新包沿用源包的 `program`，所以那次 `open` 必然要先 `close` ——
    契约级的选择，文档只能教。
