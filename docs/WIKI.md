...
- **评测回归网**：一组任务（改一个属性并确认、按表加字段、加页签、定位一个只读区为何不能改…）
  ...
  **三轮都跑完了**（2026-09-24 基线 / 2026-09-25 引擎批次之后 / 同日 ①② 与 Go 侧那批之后），
  装置与提示词在 `tools/eval/`，全部结果与摩擦记录在 `docs/eval-baseline.md`。
  第三轮第一次有了**可比的方向**：调用数 62 → 52（四个任务里三个降），而唯一上升的那个
  （E1）是因为**文档自己自相矛盾**（§7 的 `net108` 段）—— "不是缺信息，是信息打架"这一类
  是评测目前最值钱的产出。第三轮的 13 条摩擦已当场修掉（该文档的 F17–F29）。
  第二轮沿这条线挖出的两件事**都已做掉**：
