...
编号格式 `std-00001`/`azz-00041`/`lib-xxxxx`；负整数（SQLCODE 如 `-100`）需 `--` 分隔：
`tt dict msg -- -100`。返回**文本**、**建议处理**、建议作业、技术细节、**类型**（0=警告/1=错误/
2=资讯）、状态。消息按语言各一条：默认只显示 `--lang` 行，该编号无此语言时命令会列出可用语言
（与源系统一致：精确匹配、无自动回退）。
...
远程直查使用客户端驱动直连 `db.host:port`（账号取列表首项），不要求本地已 sync；金仓与 Oracle
