...
tt dev tzs …   设计器表单包（.tzs/.tzv）：53 个具名动词 + JSON 参数（field_add 任务级 + open/save/set_spec_attr…），
               由设计器自己的引擎读写，不是拼 XML
...
| [7. `tt dev tzs` 表单包](#7-tt-dev-tzs-表单包) | 引擎与设计器程序集、53 个动词（含任务级 `field_add`）、寻址与基线规则 | 用或改表单读写时 |
...
| `<动词> --args '<JSON 对象>'` | 读写表单，**唯一写路径**（**53 个具名动词**，见下） | 是 | 只经 `save` 写到**新**包 |
...
53 个动词由引擎的函数表在运行时生成：`tt dev tzs <动词>` 里，动词名 = manifest 的 `fn`，
...
| 项 | 从前 | 现在 | 依据 |
|---|---|---|---|
| 枚举动词 | `tt dev tzs fns` 打全表（名字 + 全部参数 + 类型 + 必填 + 取值集） | `tt dev tzs --help` 打**索引**（分组 + 名字）；敲错名字打同一份清单 | 参数是每个动词自己的契约，按需问 `<动词> --help`；一次摊开 53 个动词的参数只会淹没人 |
...
与第 17 条（`list_packages`）是第三轮评测逼出来的。全部同日发。这一批至此全部完成。**
